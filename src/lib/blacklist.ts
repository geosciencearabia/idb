import blacklistCsv from "@/data/blacklist.csv?raw";
...
const parseCsv = () => {
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    entries.push({
      ...
      workId: normalizeId(workId) || undefined,
      doi: normalizeId(doi) || undefined,
...
const normalizeWorkId = (work: Work) => normalizeId(work.workId);
const normalizeDoiValue = (work: Work) => normalizeId(work.doi);
